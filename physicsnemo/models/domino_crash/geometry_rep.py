...
class GeoConvOut(nn.Module):
    ...
    def __init__(
        ...
        input_features = input_features + self.nodal_geometry_features
    ...
    def forward(
        ...
        Args:
            ...
            geometry_features: Geometry features tensor
                (batch_size, nx*ny*nz, n_points, n_features)
        Returns:
        ...
        if self.nodal_geometry_features > 0:
            x = torch.cat((x, geometry_features), axis=-1)

        x = rearrange(
            x, "b x y z -> b x (y z)", x=nx * ny * nz, y=self.neighbors_in_radius, z=3+self.nodal_geometry_features
        )
...
class GeometryRep(nn.Module):
    ...
    def forward(
        self, x: torch.Tensor, p_grid: torch.Tensor, sdf: torch.Tensor, geometry_features: torch.Tensor | None = None
    ) -> torch.Tensor:
        """
        ...
        if self.geo_encoding_type == "both" or self.geo_encoding_type == "stl":
            ...
            for j in range(len(self.radii)):
                ...
                if geometry_features is not None:
                    geometry_features_calculated = torch.unsqueeze(geometry_features[0, mapping[0]], 0)
                else:
                    geometry_features_calculated = None
                x_encoding_inter = self.geo_conv_out[j](k_short, p_grid, geometry_features=geometry_features_calculated)
