    ...
    def forward(
        ...
        for f in range(self.num_variables):
            for p in range(self.num_sample_points):
                if p == 0:
                    surface_m_c = surface_mesh_centers
                else:
                    surface_m_c = surface_mesh_neighbors[:, :, p - 1] + 1e-6
                    noise = surface_mesh_centers - surface_m_c
                    dist = torch.norm(noise, dim=-1, keepdim=True)

                if surface_features is not None:
                    surface_m_c = torch.cat((surface_m_c, surface_features), dim=-1)
